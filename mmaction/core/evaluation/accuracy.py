...
def precision(scores, labels):
    return compute_precision_recall(scores, labels, np.sum(labels))[0]


def recall(scores, labels):
    return compute_precision_recall(scores, labels, np.sum(labels))[1]


def fhalf_score(scores, labels):
    prec = precision(scores, labels)
    rec = recall(scores, labels)

    fhalf = (1 + 0.5 ** 2) * (prec * rec) / (((0.5 ** 2) * prec) + rec)
    return fhalf
